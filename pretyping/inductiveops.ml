...
let squash_elim_sort sigma squash rtnsort =
  ...
  let add_unif_if_cannot_elim_into starget =
    if Inductive.sort_eliminates_to
	(UState.qgraph @@ Evd.ustate sigma) starget @@
	 ESorts.kind sigma rtnsort
    then sigma
...
let make_allowed_elimination_actions sigma s =
  ...
  ; squashed_to_quality =
      fun indq -> let sq = EConstr.ESorts.quality sigma s in
	       if Inductive.eliminates_to (UState.qgraph @@ Evd.ustate sigma) indq sq
	       then Some sigma
	       else
