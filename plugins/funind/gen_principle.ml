    ...
    LocalAssum
      ...
      , let args, ty =
        ...
        Global.add_constraints @@
          UnivSubst.enforce_elim_to_sort s toSort PolyConstraints.empty;
...
let generate_functional_principle (evd : Evd.evar_map ref) old_princ_type sorts
  ...
  try
    let f = funs.(i) in
    let sigma, type_sort = Evd.fresh_sort_quality !evd Quality.qtype in
    evd := sigma;
    let new_sorts =
      match sorts with
    ...
    let hook new_principle_type _ =
      if Option.is_empty sorts then (
        ...
        let register_with_sort fam_sort =
          ...
          let evd', s = Evd.fresh_sort_quality evd' fam_sort in
          let name =
          ...
          names := name :: !names
        in
        register_with_sort Quality.qprop )
    in
      ...
      let _ =
        List.map_i
          (fun i _x ->
            ...
            let princ = Indrec.lookup_eliminator env (ind_kn, i) Quality.qprop in
...
let make_scheme evd (fas : (Constr.pconstant * Quality.t) list) : _ list =
    ...
    (fun () ->
        ...
        with Not_found ->
          Array.of_list
            (List.map
               ...
               (make_scheme evd
                  (Array.map_to_list (fun const -> (const, Quality.qtype)) funs)))
      in
      let proving_tac =
      ...
      let sigma, scheme =
        let sigma, inds = CArray.fold_left_map_i (fun i sigma _ ->
            let sigma, s = Evd.fresh_sort_quality ~rigid:UnivRigid sigma (Quality.qtype) in
            sigma, (((kn, i), u), true, s))
            !evd
            mib.mind_packets
        in
...
let build_case_scheme fa =
  ...
  let sorts = (fun (_, _, x) -> EConstr.ESorts.make @@ fst @@ UnivGen.fresh_sort_quality x) fa in
