...
module QState : sig
  ...
  val merge_constraints : QGraph.constraint_source -> ElimConstraints.t -> t -> t
end =
struct
...
let merge_constraints src cstrs m =
  { m with elims = QGraph.merge_constraints src cstrs m.elims }
end
...
let add_elim_local cst local =
  { local with
    local_sorts = QState.merge_constraints QGraph.Internal (ElimConstraints.singleton cst) local.local_sorts }
...
let add_constraints src uctx cstrs =
  ...
  { uctx with
    ...
    sort_variables = QState.set_elims
                       (merge_elim_constraints src uctx (PolyConstraints.qualities local') (QState.elims sorts))
                       sorts;
    minim_extra = extra; }
...
let add_elim_constraints src uctx cstrs =
  ...
  add_constraints src uctx cstrs
...
let check_elim_implication uctx cstrs cstrs' =
  ...
  let grext = merge_elim_constraints QGraph.Internal uctx cstrs g in
...
let merge_sort_variables ?loc ~sideff uctx src qvars csts =
  ...
  in
  let sort_variables = QState.merge_constraints src csts sort_variables in
  { uctx with sort_variables; names }

let merge_sort_context ?loc ~sideff rigid src uctx ((qvars,levels),csts) =
  let uctx = merge_sort_variables ?loc ~sideff uctx src qvars (PolyConstraints.qualities csts) in
...
let check_poly_decl_rev uctx decl =
  ...
  let uctx, elim_csts =
    ...
    then uctx, elim_csts
    else restrict_elim_constraints QGraph.Rigid uctx decl.polydecl_elim_constraints,
         elim_csts
  in
  let uctx' = UContext.make nas (inst, (elim_csts,univ_csts)) in
  uctx, uctx'
