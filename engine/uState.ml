...
let eliminates_to_prop q m = QGraph.eliminates_to_prop m.elims (QVar q)
...
let add_qvars qmap qs =
  ...
  let fold v g =
    let g = QGraph.add_quality (QVar v) g in
    QGraph.enforce_eliminates_to (QVar v) qprop g in
...
let union ~fail s1 s2 =
  ...
  in
  let extra = !extra in
  let elims = add_qvars qmap (QVar.Set.union (QGraph.qvar_domain s1.elims) (QGraph.qvar_domain s2.elims)) in
  let s = { rigid = QSet.union s1.rigid s2.rigid; qmap; elims } in
...
let add ~check_fresh ~rigid q m =
  ...
  { rigid = if rigid then QSet.add q m.rigid else m.rigid;
    ...
    elims =
      try QGraph.add_quality (QVar q) m.elims
      with e -> if check_fresh then raise e else m.elims }

let of_set qs =
  let empty_qmap = QMap.bind (fun _ -> None) qs in
  { rigid = QSet.empty; qmap = empty_qmap;
    elims = QVar.Set.fold (fun v -> QGraph.add_quality (QVar v)) qs QGraph.initial_graph }

let of_elims elims =
  let empty_qmap = QMap.bind (fun _ -> None) (QGraph.qvar_domain elims) in
  { rigid = QSet.empty; qmap = empty_qmap; elims }
...
type t =
 { names : univ_names; (** Printing/location information *)
   local : PolyConstraints.ContextSet.t; (** The local graph of universes (variables and constraints) *)
...
let empty =
  { names = UnivNames.empty_binders, (QVar.Map.empty, Level.Map.empty);
    local = PolyConstraints.ContextSet.empty;
...
let is_empty uctx =
  PolyConstraints.ContextSet.is_empty uctx.local &&
...
let merge_elim_constraints uctx cstrs g =
...
let union uctx uctx' =
  if uctx == uctx' then uctx
    ...
    let local = PolyConstraints.ContextSet.union uctx.local uctx'.local in
    let names = names_union uctx.names uctx'.names in
    let newus = Level.Set.diff (PolyConstraints.ContextSet.levels uctx'.local)
                               (PolyConstraints.ContextSet.levels uctx.local) in
      ...
      { names;
        ...
        universes =
          (if local == uctx.local then uctx.universes
             ...
             let cstrsr = PolyConstraints.ContextSet.univ_constraints uctx'.local in
...
let add_univ_local cst local =
  { local with local_cst = PolyConstraints.add_univ cst local.local_cst }

let add_elim_local cst local =
  { local with
    local_sorts = QState.merge_constraints (ElimConstraints.singleton cst) local.local_sorts }

(* Constraint with algebraic on the left and a single level on the right *)
let enforce_leq_up u v local =
  let elim_csts = PolyConstraints.qualities local.local_cst in
  let univ_csts = UnivSubst.enforce_leq u (Universe.make v) @@
                    PolyConstraints.univs local.local_cst in
  { local with local_cst = PolyConstraints.make elim_csts univ_csts }
...
let process_constraints uctx cstrs =
  ...
  let unify_universes cst local =
    ...
    | QElimTo (a, b) ->
       let local' = add_elim_local (a, ElimTo, b) local in
       begin
         try let local' = unify_quality univs CUMUL (mk b) (mk a) local' in
             { local with local_sorts = local'.local_sorts }
         with _ -> local'
       end
    | ULe (l, r) ->
...
let add_constraints uctx cstrs =
  ...
  { uctx with
    ...
    univ_variables = vars;
    universes = merge_univ_constraints uctx (PolyConstraints.univs local') uctx.universes;
    sort_variables = QState.set_elims
                       (merge_elim_constraints uctx (PolyConstraints.qualities local') (QState.elims sorts))
                       sorts;
    minim_extra = extra; }
...
let add_univ_constraints uctx cstrs =
  ...
  add_constraints uctx cstrs

let problem_of_elim_constraints cstrs =
  ElimConstraints.fold (fun (l,d,r) cstrs ->
      match d with
      | Equal -> UnivProblem.Set.add (QEq (l,r)) cstrs
      | ElimTo -> UnivProblem.Set.add (QElimTo (l,r)) cstrs)
    cstrs UnivProblem.Set.empty

let add_elim_constraints uctx cstrs =
  let cstrs = problem_of_elim_constraints cstrs in
  add_constraints uctx cstrs

let add_poly_constraints uctx (qcstrs,ucstrs) =
  let cstrs = problem_of_univ_constraints ucstrs in
  let cstrs' = problem_of_elim_constraints qcstrs in
  let cstrs = UnivProblem.Set.union cstrs cstrs' in
  add_constraints uctx cstrs
...
let default_poly_decl =
  { polydecl_qualities = [];
    ...
    polydecl_extensible_qualities = true;
    polydecl_elim_constraints = ElimConstraints.empty;
    polydecl_extensible_elim_constraints = true;
    polydecl_instance = [];
    polydecl_extensible_instance = true;
    polydecl_univ_constraints = UnivConstraints.empty;
    polydecl_extensible_univ_constraints = true }

let poly_decl_csts decl =
  PolyConstraints.make decl.polydecl_elim_constraints decl.polydecl_univ_constraints
...
let universe_context_inst decl qvars levels names =
  let leftqs = List.fold_left (fun acc l -> QVar.Set.remove l acc) qvars decl.polydecl_qualities in
  let leftus = List.fold_left (fun acc l -> Level.Set.remove l acc) levels decl.polydecl_instance in
  let () =
    let unboundqs = if decl.polydecl_extensible_qualities then QVar.Set.empty else leftqs in
    let unboundus = if decl.polydecl_extensible_instance then Level.Set.empty else leftus in
  ...
  let instq = Array.append
      (Array.map_of_list (fun q -> Quality.QVar q) decl.polydecl_qualities)
      leftqs
  in
  let instu = Array.append (Array.of_list decl.polydecl_instance) leftus in
...
let check_elim_implication uctx cstrs cstrs' =
  let g = elim_graph uctx in
  let g = QVar.Set.fold (fun q -> QGraph.add_quality (QVar q)) (QGraph.qvar_domain g) QGraph.initial_graph in
  let grext = merge_elim_constraints uctx cstrs g in
...
let check_template_poly_decl uctx ~template_qvars decl =
  let () =
    match List.filter (fun q -> not @@ QVar.Set.mem q template_qvars) decl.polydecl_qualities with
    | (_ :: _) as qvars ->
      CErrors.user_err
  ...
  let () =
    let prefix = decl.polydecl_instance in
    if not decl.polydecl_extensible_instance
    then check_universe_context_set ~prefix levels uctx.names
  in
  if decl.polydecl_extensible_univ_constraints then uctx.local
  else begin
    check_implication uctx
      (poly_decl_csts decl) csts;
    levels, (decl.polydecl_elim_constraints,decl.polydecl_univ_constraints)
  end

let check_mono_poly_decl uctx decl =
  (* Note: if [decl] is [default_univ_decl], behave like [uctx.local] *)
  let () =
    if not (List.is_empty decl.polydecl_qualities)
...
let check_poly_univ_decl uctx decl =
  ...
  let univ_csts = if decl.polydecl_extensible_univ_constraints then univ_csts
    else begin
      check_univ_implication uctx
        decl.polydecl_univ_constraints
        univ_csts;
      decl.polydecl_univ_constraints
    end
  in
  let elim_csts = if decl.polydecl_extensible_elim_constraints then elim_csts
    else begin
      check_elim_implication uctx
        decl.polydecl_elim_constraints
        elim_csts;
      decl.polydecl_elim_constraints
    end
  in
...
let check_poly_decl ~poly uctx decl =
  let (binders, _) = uctx.names in
  let entry =
    if poly then Polymorphic_entry (check_poly_univ_decl uctx decl)
    else Monomorphic_entry (check_mono_poly_decl uctx decl) in
  entry, binders
...
let restrict_universe_context (univs, csts) keep =
  ...
  let elim_csts = PolyConstraints.qualities csts in
  let univ_csts = PolyConstraints.univs csts in
  ...
  (Level.Set.inter univs keep, PolyConstraints.make elim_csts univ_csts)
...
let restrict_elim_constraints uctx csts =
  let levels, (elim_csts,univ_csts) = uctx.local in
  let qs = QGraph.qvar_domain (QState.elims uctx.sort_variables) in
  let g = Quality.QVar.Set.fold (fun v -> QGraph.add_quality (QVar v)) qs QGraph.initial_graph in
  let uctx' = { uctx with local = (levels,(ElimConstraints.empty,univ_csts));
                          sort_variables = QState.set_elims g uctx.sort_variables } in
  add_elim_constraints uctx' csts
...
let merge ?loc ~sideff rigid uctx uctx' =
  let levels = PolyConstraints.ContextSet.levels uctx' in
  let local = PolyConstraints.ContextSet.append uctx' uctx.local in
  let declare g =
  ...
  let universes = merge_univ_constraints uctx (PolyConstraints.ContextSet.univ_constraints uctx') univs in
  let uctx =
    match rigid with
    | UnivRigid -> uctx
    | UnivFlexible b ->
...
let merge_sort_variables ?loc ~sideff uctx qvars csts =
  let sort_variables =
...
let merge_sort_context ?loc ~sideff rigid uctx ((qvars,levels),csts) =
  let uctx = merge_sort_variables ?loc ~sideff uctx qvars (PolyConstraints.qualities csts) in
...
let add_universe ?loc name strict uctx u =
  ...
  let local = PolyConstraints.ContextSet.add_level u uctx.local in
  let names =
    match name with
...
let from_env ?(binders=[]) env =
  make_with_initial_binders (Environ.universes env) (Environ.qualities env) binders
...
let minimize uctx =
  ...
  let (vars', us') =
    normalize_context_set uctx.universes uctx.local uctx.univ_variables
      uctx.minim_extra
  in
  if PolyConstraints.ContextSet.equal us' uctx.local then uctx
  else
...
let universe_context_inst_decl decl qvars levels names =
  ...
  let () =
    ...
    then error_unbound_universes unboundqs unboundus names
  in
  let instq = Array.map_of_list (fun q -> Quality.QVar q) decl.polydecl_qualities in
  let instu = Array.of_list decl.polydecl_instance in
  let inst = Instance.of_array (instq,instu) in
  inst

let check_poly_decl_rev uctx decl =
  let levels, (elim_csts,univ_csts as csts) = uctx.local in
  let qvars = QState.undefined uctx.sort_variables in
  let inst = universe_context_inst_decl decl qvars levels uctx.names in
  let nas = compute_instance_binders uctx inst in
  let () = check_implication uctx csts (poly_decl_csts decl)
  in
  let uctx = fix_undefined_variables uctx in
  let uctx, univ_csts =
    if decl.polydecl_extensible_univ_constraints
    then uctx, univ_csts
    else restrict_univ_constraints uctx decl.polydecl_univ_constraints,
         univ_csts
  in
  let uctx, elim_csts =
    if decl.polydecl_extensible_elim_constraints
    then uctx, elim_csts
    else restrict_elim_constraints uctx decl.polydecl_elim_constraints,
         elim_csts
  in
  let uctx' = UContext.make nas (inst, (elim_csts,univ_csts)) in
  uctx, uctx'
  ...
  if is_empty ctx then mt ()
    ...
    v 0
      (str"UNIVERSES:"++brk(0,1)++
       h (PolyConstraints.ContextSet.pr prq prl (context_set ctx)) ++ fnl () ++
