    ...
    let c =
      ...
      | Ind (ind_sp,u) ->
        ...
        | Some false, Some _ ->
          ...
          | None ->
             let sort = UnivGen.QualityOrSet.of_quality sort in
        ...
        | _ ->
           ...
           | Some r -> destConstRef r
           | None -> destConstRef (lookup_eliminator env ind_sp @@
                                    UnivGen.QualityOrSet.of_quality sort)
           end
        end
      | _ ->
    ...
    let keep =
      if keep_head_inductive sigma ty1 then true
        ...
        let s = UnivGen.QualityOrSet.of_quality @@
                  get_sort_quality_of env sigma ty1 in
...
let discr_positions env sigma { eq_data = (lbeq,(t,t1,t2)); eq_term = v; eq_evar = evs } cpath dirn =
  ...
  let false_kind = UnivGen.QualityOrSet.of_quality @@ Retyping.get_sort_quality_of env sigma false_0 in
