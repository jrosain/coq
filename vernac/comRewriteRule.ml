...
let interp_rule (udecl, lhs, rhs: Constrexpr.poly_decl_expr option * _ * _) =
  let env = Global.env () in
  ...
  let evd, udecl =
    ...
    let decl = {
      ...
      polydecl_extensible_univ_constraints = udecl.polydecl_extensible_univ_constraints;
    } in
    evd, decl
  in
  let nvarqs = List.length udecl.polydecl_qualities in
  let nvarus = List.length udecl.polydecl_instance in
  ...
  let evd = Evd.restrict_universe_context evd uvars in
  let uctx, uctx' = UState.check_poly_decl_rev (Evd.ustate evd) udecl in
