...
module type StagedModS = sig
  ...
  val load_keep : int -> DirPath.t -> ModPath.t -> keep_objects -> unit
  val load_escape : int -> DirPath.t -> ModPath.t -> escape_objects -> unit
...
let check_sub mp mtb sub_mtb_l =
  ...
  in
  let cst, _ = List.fold_right fold sub_mtb_l (PolyConstraints.empty, Global.env ()) in
...
let build_subtypes env mp args mtys =
  let (ctx, ans) = List.fold_left_map
    (fun ctx (mte,base,kind,inl) ->
       ...
       let state = ((Environ.universes env, PolyConstraints.empty), Reductionops.inferred_universes) in
       let mtb, (_, cst), _ = Mod_typing.translate_modtype state vm_state env mp inl (args,mte) in
       let ctx = PolyConstraints.ContextSet.add_constraints cst ctx in
       ctx, mtb)
...
let intern_arg (acc, cst) (mbidl,(mty, base, kind, inl)) =
  ...
  let () =
    let state = ((Global.universes (), PolyConstraints.empty), Reductionops.inferred_universes) in
