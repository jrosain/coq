...
let pr_polydecl_qualities l extensible =
  (* "extensible" not really supported in syntax currently *)
  if List.is_empty l then mt()
  else prlist_with_sep spc pr_lident l ++ strbrk " | "

let pr_polydecl_instance l extensible =
  prlist_with_sep spc pr_lident l ++
  (if extensible then str"+" else mt ())

let pr_cumul_polydecl_instance l extensible =
  prlist_with_sep spc pr_variance_lident l ++
  (if extensible then str"+" else mt ())

let pr_polydecl_constraints elims elim_extensible univs univ_extensible =
  if List.is_empty elims && elim_extensible && List.is_empty univs && univ_extensible then mt ()
  else pr_spcbar () ++ prlist_with_sep pr_comma pr_elim_constraint elims ++
         (if elim_extensible then str"+" else mt()) ++
         prlist_with_sep pr_comma pr_uconstraint univs ++
         (if univ_extensible then str"+" else mt())
...
let pr_universe_decl l =
  ...
  | Some l ->
    str"@{" ++
    pr_polydecl_qualities l.polydecl_qualities l.polydecl_extensible_qualities ++
    pr_polydecl_instance l.polydecl_instance l.polydecl_extensible_instance ++
    pr_polydecl_constraints l.polydecl_elim_constraints l.polydecl_extensible_elim_constraints l.polydecl_univ_constraints l.polydecl_extensible_univ_constraints ++
    str "}"
...
let pr_cumul_univ_decl l =
  ...
  | Some l ->
    str"@{" ++
    pr_polydecl_qualities l.polydecl_qualities l.polydecl_extensible_qualities ++
    pr_cumul_polydecl_instance l.polydecl_instance l.polydecl_extensible_instance ++
    pr_polydecl_constraints l.polydecl_elim_constraints l.polydecl_extensible_elim_constraints l.polydecl_univ_constraints l.polydecl_extensible_univ_constraints ++
    str "}"
...
let pr_onescheme (idop, {sch_type; sch_qualid; sch_sort}) =
  ...
  hov 0 str_identifier ++ spc () ++ hov 0 (str_scheme ++ spc() ++ pr_smart_global sch_qualid)
    ++ spc () ++ hov 0 (keyword "Sort" ++ spc() ++ UnivGen.QualityOrSet.pr Quality.QVar.raw_pr sch_sort)
