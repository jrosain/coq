...
struct
  ...
  let empty = (empty_bound_names, (Instance.empty, PolyConstraints.empty))
  let is_empty (_, (univs, csts)) = Instance.is_empty univs && PolyConstraints.is_empty csts

  let pr prq prl ?variance (_, (univs, csts) as uctx) =
    if is_empty uctx then mt() else
      h (Instance.pr prq prl ?variance univs ++ str " |= ") ++ h (v 0 (PolyConstraints.pr prq prl csts))
  ...
  let hcons ({quals = qnames; univs = unames}, (univs, csts)) =
    ...
    let hcsts, csts = PolyConstraints.hcons csts in
  ...
  let union (names, (univs, csts)) (names', (univs', csts')) =
    append_bound_names names names', (Instance.append univs univs', PolyConstraints.union csts csts')
