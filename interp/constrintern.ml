...
let intern_qvar ~local_univs = function
  ...
  | CQVar qid ->
    ...
    | Some u -> GQVar u
    | None ->
      try GQVar (Quality.QVar.make_global (Nametab.locate_quality qid))
      with Not_found ->
        if is_id && local_univs.unb_univs
...
let interp_poly_decl env decl =
  let open UState in
  let evd = Evd.from_env env in
  let evd, qualities = List.fold_left_map (fun evd lid ->
      Evd.new_quality_variable ?loc:lid.loc ~name:lid.v evd)
      evd
      decl.polydecl_qualities
  in
  let evd, instance = List.fold_left_map (fun evd lid ->
      Evd.new_univ_level_variable ?loc:lid.loc univ_rigid ~name:lid.v evd)
      evd
      decl.polydecl_instance
  in
  let evd, univ_cstrs = interp_univ_constraints env evd decl.polydecl_univ_constraints in
  let evd, elim_cstrs = interp_elim_constraints env evd decl.polydecl_elim_constraints in
  let decl = {
    polydecl_qualities = qualities;
    polydecl_extensible_qualities = decl.polydecl_extensible_qualities;
    polydecl_elim_constraints = elim_cstrs;
    polydecl_extensible_elim_constraints = decl.polydecl_extensible_elim_constraints;
    ...
    polydecl_extensible_univ_constraints = decl.polydecl_extensible_univ_constraints;
  }
  in evd, decl

let interp_cumul_poly_decl env decl =
  let open UState in
  let binders = List.map fst decl.polydecl_instance in
  let variances = Array.map_of_list snd decl.polydecl_instance in
  let evd = Evd.from_env env in
  let evd, qualities = List.fold_left_map (fun evd lid ->
      Evd.new_quality_variable ?loc:lid.loc ~name:lid.v evd)
      evd
