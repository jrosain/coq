...
let rec check_mexpr env opac mse mp_mse res = match mse with
  ...
  | MEapply (f,mp) ->
    ...
    let state = ((Environ.universes env, Environ.qualities env), Conversion.checked_universes) in
    let _ : UGraph.t * QGraph.t = Subtyping.check_subtypes state env mp mtb (MPbound farg_id) farg_b in
...
let rec check_module env opac mp mb opacify =
  ...
  | Some (sign,delta) ->
    ...
    let state = ((Environ.universes env, Environ.qualities env), Conversion.checked_universes) in
    let _ : UGraph.t * QGraph.t = Subtyping.check_subtypes state env mp mtb1 mp mtb2 in
    ()
  in
  opac
