...
let import senv opac clib vmtab digest =
  ...
  let env = push_quality_set (Safe_typing.qualities_of_library clib) env in
  let cstrs = Safe_typing.univs_of_library clib in
  let cstrs = PolyConstraints.ContextSet.filter_out_constant_qualities cstrs in
  let env = push_context_set QGraph.Rigid ~strict:true cstrs env in
